if (grid_col < 0 || grid_col >= global.grid_cols || grid_row < 0 || grid_row >= global.grid_rows) {
	...
	if global.grid_terrains[grid_row][grid_col].type == "water"{
		if sprite_index == spr_infected_duck_mouse_land or sprite_index == spr_infected_duck_mouse_land_helmet{
		...
		if sprite_index == spr_infected_duck_mouse or sprite_index == spr_infected_duck_mouse_helmet{
			state = ENEMY_STATE.ACTING
			sprite_index = spr_infected_duck_mouse_enter
			timer = 0
			audio_play_sound(snd_enter_water,0,0)
